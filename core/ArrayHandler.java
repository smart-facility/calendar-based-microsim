...
public class ArrayHandler {
	 ...
	 * @return
	 */
	public static ArrayList<Integer> pickRandomFromList(ArrayList<Integer> myList, int nPick) {
		ArrayList<Integer> picked = new ArrayList<Integer>();
		Collections.shuffle(myList, HardcodedData.random);
		for (int i=0; i<nPick; i++) {
			picked.add(myList.get(i));
		}
		return picked;
	}
	 ...
	 * @return
	 */
	public static Object pickARandomElementFromSet(Set<?> srcSet) {
		int randIndex = HardcodedData.random.nextInt(srcSet.size());
		int i = 0;
		for (Object obj : srcSet) {
			if (i==randIndex) {
				return obj;
			}
			i += 1;
